					...
					{data.map(({ node }) => (
						<li key={node.id} className={styles.item}>
							<div className={styles.content}>
								<p>
									{dateFormat === 'year' && (
										<span className={styles.dates}>
											{new Date(node.start).getFullYear()} &ndash;{' '}
											{new Date(node.end).getFullYear()}
										</span>
									)}
									{dateFormat === 'full' &&
										new Date(node.start).getMonth() ===
											new Date(node.end).getMonth() && (
											<span className={styles.dates}>
												{new Date(node.start).toLocaleDateString(
													'fr-FR',
													dateOptions
												)}
											</span>
									...
									{dateFormat === 'full' &&
										new Date(node.start).getMonth() !==
											...
											<span className={styles.dates}>
												...
												&ndash;{' '}
												{new Date(node.end).toLocaleDateString(
													'fr-FR',
													dateOptions
												)}
											</span>
								...
								{node.school && (
									<h3>
										<span className={styles.entity}>{node.school}</span>,{' '}
										{node.location}
									</h3>
								)}
